...
const fakeContacts = [ { id: 'f1', nickname: 'Luca23', last_seen: new Date(Date.now() - 30 * 1000).toISOString(), // online }, { id: 'f2', nickname: 'Anna_Design', last_seen: new Date(Date.now() - 5 * 60 * 1000).toISOString(), // offline }, { id: 'f3', nickname: 'Marco.Tech', last_seen: new Date(Date.now() - 20 * 1000).toISOString(), }, ];

const fakeMessages = [ { id: 1, sender_id: 'me', receiver_id: 'f1', content: 'Ciao Luca, ti occupi anche di piccoli lavori?', }, { id: 2, sender_id: 'f1', receiver_id: 'me', content: 'Certo! Di cosa hai bisogno?', }, { id: 3, sender_id: 'me', receiver_id: 'f1', content: 'Una parete da imbiancare. Ti va?', }, { id: 4, sender_id: 'f1', receiver_id: 'me', content: 'Perfetto, ci organizziamo!', }, ];
...
const fetchConversations = async () => { const { data, error } = await supabase.rpc('get_conversations', { current_user_id: user.id, }); if (!error && data.length > 0) setConversations(data); else setConversations(fakeContacts); };
...
return ( <div className="flex h-screen text-white bg-[#0f1e3c]"> {/* Sidebar */} <div className="w-1/3 border-r border-gray-800 p-4 overflow-y-auto hidden md:block"> <h2 className="text-xl font-semibold mb-4">Messaggi</h2> {conversations.map((profile) => ( <div key={profile.id} onClick={() => setSelectedUser(profile)} className={p-3 cursor-pointer rounded mb-2 flex items-center gap-3 transition ${ selectedUser?.id === profile.id ? 'bg-yellow-600 text-black' : 'hover:bg-gray-700' }} > <span className={w-3 h-3 rounded-full ${ isOnline(profile.last_seen) ? 'bg-green-400' : 'bg-gray-500' }} ></span> <span className="font-medium">{profile.nickname}</span> </div> ))} </div>
