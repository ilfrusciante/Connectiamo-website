...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const geoResponse = await fetch(`https://secure.geonames.org/postalCodeLookupJSON?postalcode=${cap}&placename=${city}&maxRows=1&username=RobyRob`);
      const geoData = await geoResponse.json();

      if (!geoData.postalcodes || geoData.postalcodes.length === 0) {
        setError(`Errore: CAP ${cap} e città "${city}" non trovati.`);
        return;
      }

      const geoCity = geoData.postalcodes[0].placeName;
      if (!normalize(geoCity).includes(normalize(city))) {
        setError(`Il CAP ${cap} non corrisponde alla città inserita (${city}).`);
        return;
      }
    } catch (err) {
      setError('Errore nella verifica del CAP con GeoNames.');
      return;
    }
    ...
    <div className="min-h-screen bg-[#0f1e3c] text-white flex items-center justify-center px-4">
      <div className="max-w-lg w-full p-6">
        <div className="w-full flex justify-center mb-6">
          <Image src="/images/illustration-signup.png" alt="Registrazione" width={0} height={0} sizes="100vw"
            style={{ width: '100%', maxWidth: '360px', height: 'auto' }} className="rounded-md" />
        </div>
